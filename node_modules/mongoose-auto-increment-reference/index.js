...
if (process.env.incrementVersion !== 'v1') {
...
exports.plugin = function (schema, options) {
  ...
  var settings = {
    ...
    unique: false // Should we create a unique index for the field
  },
  ...
  var resetCount = function (callback) {
    IdentityCounter.findOneAndUpdate(
      { model: settings.model, field: settings.field, reference: settings.reference },
  ...
  schema.pre('save', function (next) {
    ...
    if (doc.isNew) {
      ...
      (function save() {
        ...
        // first time.
          let referenceValue = doc[settings.reference] 
          ...
          if (typeof doc[settings.field] === 'number') {
            IdentityCounter.findOneAndUpdate(
              ...
              { model: settings.model, field: settings.field, reference: referenceValue, count: { $lt: doc[settings.field] } },
            ...
            counter(settings, referenceValue, doc, next)
          }
...
const counter = (settings, referenceValue, doc, next) => {
  IdentityCounter.findOneAndUpdate(
    // IdentityCounter documents are identified by the model and field that the plugin was invoked for.
    { model: settings.model, field: settings.field, reference: referenceValue },
    ...
    function (err, updatedIdentityCounter) {
      ...
      if (updatedIdentityCounter) {
        ...
        IdentityCounter.create({ model: settings.model, field: settings.field, reference: referenceValue, startAt: settings.startAt,  count: settings.startAt - settings.incrementBy })
